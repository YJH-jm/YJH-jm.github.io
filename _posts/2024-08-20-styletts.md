...
StyleTTS의 목적은 입력으로 들어가는 phonemes (음소) $\boldsymbol{t} \in \mathcal{T}$ 와 reference mel-spectogram $\boldsymbol{x} \in \mathcal{X}$ 가 있을 때, $\boldsymbol{x}$ 의  일반화된 음성 스타일이 잘 반영되어 있는 $\boldsymbol{x}$에 대응되는  mel-spectrogram $\tilde{\boldsymbol{x}} \in \mathcal{X}$ 를 생성할 수 있도록 학습하는 것
...
- 속도를 변경하는 과정에서 단순히 음성을 빠르게 하거나 느리게 재생하면, pitch 변화
- 녹음을 빠르게 재생하면 목소리가 헬륨을 마신 것처럼 높아지며, 반대로 느리게 재생하면 목소리가 저음으로 변하
- 에너지도 변화

이런 접근이 duration이 맞지 않을 때 부자연스러운 prosody가 생성되는 것을 방지함

이 때, $\mathrm{f} 0$와 energy reconstruction error는 $$\mathcal{L}_{\mathrm{f} 0}$$ 와 $$\mathcal{L}_{n} $$ 각각 계산
...
- $$\hat{n}=P_{n}\left(S\left(\boldsymbol{h}_{\text {text }}, s\right) \cdot \tilde{\boldsymbol{a}}_{\text {align }}\right)$$ : 예측한 energy
